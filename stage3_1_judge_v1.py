...
async def judge_v1_website_task(model_name: str, app_name: str, progress_tracker, experiment_name: str = "exp1", run_key: str = None, v0_dir: str = None, **kwargs) -> dict:
    """单个修订版网站 judge 任务"""
    try:
        ...
        # 构建v1网站路径 (run_key layout)
        v1_website_path = Path(f"experiments/{experiment_name}/runs/{run_key}/stage3_0/{app_name}/{model_name}/v1_website/index.html")
        
        if not v1_website_path.exists():
            return {
                'success': False,
                'error': f"Revised website not found: {v1_website_path}",
                'model': model_name,
                'app': app_name
            }
        
        with open(v1_website_path, 'r', encoding='utf-8') as f:
            html_content = f.read()
        ...
        if analysis_result['success']:
            ...
            # 简要比较v0和v1的支持度（可选统计）
            if v0_dir:
                v0_rules_path = Path(f"v0/{v0_dir}/tasks/{app_name}/states/{model_name}/rules.json")
            else:
            ...
            return {
                ...
                'v0_supported_count': v0_supported_count,
                'v1_supported_count': v1_supported_count,
...
async def main():
    ...
    for model in models:
        for app in apps:
            ...
            if v1_website_path.exists() and tasks_path.exists():
    ...
    summary = await runner.run_parallel_tasks(
        models=valid_models,
        apps=valid_apps,
        task_func=judge_v1_website_task,
    ...
    for result in summary['results']:
        if result['result'].get('success'):
            ...
            total_v0_supported += result_data.get('v0_supported_count', 0)
            total_v1_supported += result_data.get('v1_supported_count', 0)
    ...
    detailed_summary = {
        **summary,
        ...
        'total_v0_supported': total_v0_supported,
        'total_v1_supported': total_v1_supported,
        'total_improvement': total_improvement,
        'total_tasks_analyzed': total_tasks,
        'v0_support_rate': total_v0_supported / total_tasks if total_tasks > 0 else 0,
        'v1_support_rate': total_v1_supported / total_tasks if total_tasks > 0 else 0
    }
    ...
    for result in summary['results']:
        if result['result'].get('success'):
            model = result['model']
            app = result['app']
            v0_supported = result['result'].get('v0_supported_count', 0)
            v1_supported = result['result'].get('v1_supported_count', 0)
            improvement = result['result'].get('improvement', 0)
            total = result['result'].get('total_tasks', 0)
            
            if model not in model_stats:
                model_stats[model] = {
                    'v0_supported': 0, 'v1_supported': 0, 'improvement': 0, 'total': 0, 'apps': []
                }
            model_stats[model]['v0_supported'] += v0_supported
            model_stats[model]['v1_supported'] += v1_supported
            model_stats[model]['improvement'] += improvement
            model_stats[model]['total'] += total
            model_stats[model]['apps'].append({
                'app': app,
                'v0_supported': v0_supported,
                'v1_supported': v1_supported,
                'improvement': improvement,
                'total': total,
                'v0_rate': v0_supported / total if total > 0 else 0,
                'v1_rate': v1_supported / total if total > 0 else 0
            })
    ...
    model_comparison = {
        'overview': {
            model: {
                'total_v0_supported': stats['v0_supported'],
                'total_v1_supported': stats['v1_supported'],
                'total_improvement': stats['improvement'],
                'total_tasks': stats['total'],
                'v0_support_rate': stats['v0_supported'] / stats['total'] if stats['total'] > 0 else 0,
                'v1_support_rate': stats['v1_supported'] / stats['total'] if stats['total'] > 0 else 0,
